...
'allow_net_wm_moveresize'::
	Let clients start a move or a resize of their own window with *_NET_WM_MOVERESIZE*, as applications that draw their own title bar do. When *false*, *_NET_WM_MOVERESIZE* is not advertised, so those clients move and resize their windows themselves. Defaults to *true*.

...
*-i*::
	Ignore all monitor updates after the initial startup detection. Equivalent to starting with the 'ignore_monitor_updates' setting already enabled.
...
*-S*, *--snap* left|right|top|bottom|top_left|top_right|bottom_left|bottom_right|maximize::
	Snap the selected window to a region of its monitor, Windows-style. The window is made floating first if it isn't already, and the monitor's padding is honoured. *maximize* sets the fullscreen state instead of resizing. This is the same geometry used by the pointer drag-to-edge gesture (see *edge_snap_enabled*), reachable from a keybinding.
...
*--prune-dead*::
	Remove any node whose window no longer exists on the server, and print how many were removed. Such a node makes every subsequent restacking request fail against a dead window id, which silently desynchronises the visible stacking order from the tree. This sweep is run automatically after a state restore; use it manually if window layering starts behaving erratically. Costs one round-trip to the X server per managed window.
...
*-a*, *--add* [*-o*|*--one-shot*] (<class_name>|\*)[:(<instance_name>|\*)[:(<name>|\*)]]|(<property>[~]=<pattern>[/i])... [*-o*|*--one-shot*] <effect>...::
...
'instance', 'name' (the title), 'type', 'role' (*WM_WINDOW_ROLE*, always empty under
the wlroots backend) and 'transient' (*on* when the window is a child of another
one). A window matches the rule when every condition holds, and a rule with no
condition at all, only effects, matches every window, the same as '\*:*:*'.

The first argument after the options decides the form. When it starts with a key
(lowercase letters and underscores) followed by '=' or '~=', the rule is a list of
conditions and effects, and a key that is neither a property nor an effect is an
error. Anything else is the pattern above, so a pattern whose title contains an
'=', such as 'st:*:vim = notes', is still a pattern. Only the first '=' of a
condition ends the property, which lets the same title be written as a condition
too: 'name=vim = notes'. The one pattern that cannot be given as such is a lone
lowercase class containing an '=', such as 'foo=bar', which reads as the unknown key
'foo': write it as 'class=foo=bar'.

With '=' the pattern is compared as written, and '\*' alone matches anything, as it
does in the pattern above. With '~=' the pattern is a POSIX extended regular
expression, where '\*' alone is not valid: use '.*', or leave the condition out.

A pattern ending in '/i' is compared ignoring case, so 'class~=^crx_/i' is both.
The properties 'type' and 'transient' take neither a regular expression nor the
'/i' suffix: they are always compared as written. 'type' is one of *normal*,
*dock*, *desktop*, *notification*, *dialog*, *utility* or *toolbar*, and
'transient' is a boolean.

A pattern holds at most 255 characters, not counting the '/i' suffix, and the
effects of a rule, together and separated by spaces, at most 255 as well.

A rule is refused when a pattern is not a valid regular expression, a condition is
repeated, a 'type' or 'transient' value is not in its list, 'type' or 'transient' is
given with '~=' or '/i', a key is neither a property nor an effect, a pattern or
the effects are too long, or the conditions are too long to be listed.
...
*-r*, *--remove* ^<n>|head|tail|(<class_name>|\*)[:(<instance_name>|\*)[:(<name>|*)]]|<conditions>...::
	Remove the given rules.

A pattern removes the rules, in either form, whose class, instance and name
patterns are the ones it gives, field by field, a '\*' in it standing for any of
them: '\*:\*:\*' removes every rule. A rule written as conditions can also be
removed by its conditions exactly as *rule -l* lists them, given as a single
argument; compared that way, a rule with one more condition, or a different
operator, is a different rule and stays.

----
bspc rule -r 'class=Google-chrome instance~=^crx_'
----
...
keybind 'OPTIONS'::
	Manage bspwm's built-in keybindings.

Built-in keybindings exist mainly for the Wayland backend, where there is no
*sxhkd* to grab keys. They work on X11 too, but note that bspwm and *sxhkd*
cannot both hold a grab on the same key: whichever registers first wins and
the other silently gets nothing. Prefer one or the other per key.

A 'COMBO' is modifiers and a key joined by *+*, e.g. *"super + shift + q"*.
Accepted modifier names are *super*/*mod4*, *alt*/*mod1*, *ctrl*/*control*,
*shift*, *mod2*, *mod3* and *mod5*. The key is an XKB keysym name and is
matched unshifted, so write *"super + shift + q"*, not *"super + Q"*. Num
Lock, Caps Lock and Scroll Lock are ignored when matching.

Options
^^^^^^^

*-a*, *--add* 'COMBO' 'COMMAND'::
	Bind 'COMBO' to 'COMMAND', which is run via *sh -c*.

*-r*, *--remove* 'COMBO'::
	Remove the binding for 'COMBO'.
...
*--load* 'PATH'::
	Load bindings from a file of alternating 'COMBO' and command lines. Blank
	lines and lines starting with *#* are ignored. This is not a full *sxhkd*
	parser: brace expansion is not supported, and a combo line is only
	recognised as such if it contains a *+*.
...
'pointer_modifier'::
	Keyboard modifier used for moving or resizing windows. Accept the following values: *shift*, *control*, *lock*, *mod1*, *mod2*, *mod3*, *mod4*, *mod5*. *alt*, *ctrl* and *super* are accepted too, as in sxhkd and 'keybind': they stand for *mod1*, *control* and *mod4*, where those keys sit in a standard keyboard map, and reading the setting back gives *mod1*, *control* or *mod4*.
...
'pointer_big_increment'::
	Strides, in pixels, of a stepped move or resize. While a window is dragged with 'pointer_modifier', holding 'pointer_increment_modifier' makes it advance in steps of 'pointer_increment', and holding 'pointer_big_increment_modifier' in steps of 'pointer_big_increment'. The steps count from where the window was when the modifier went down, to the nearest step, so an odd size stays odd and only the stride changes. Neither the edge magnet nor the edge snap zones move a window during a stepped drag. Must be at least *1*. X11 only. Default to *10* and *50*.

'pointer_increment_modifier'::
'pointer_big_increment_modifier'::
	Keyboard modifiers that turn a pointer drag into a stepped one, pressed once the drag has started. When both are held, the big step wins. Accept the same values as 'pointer_modifier', plus *none* to disable the step. X11 only. Default to *shift* and *control*.
...
'raise_floating_on_click'::
	Also raise a floating window when it is clicked, rather than only focusing it. Defaults to *true*.

'edge_snap_enabled'::
	Enable Windows-style edge snapping: dragging a window against a screen edge or corner snaps it to that region on release. See also *--snap* under *Node*. Defaults to *true*.

'edge_snap_threshold'::
	Distance in pixels from a monitor edge at which a drag enters a snap zone. Defaults to *20*.
...
'cascade_offset'::
	When bspwm centres a new floating window on its own initiative (because the window supplied no position), offset each successive one by this many pixels so they don't stack exactly on top of each other. Set to *0* to disable. Windows centred because a rule said *center=on* are never cascaded. Defaults to *20*.

'tile_limit_enabled'::
	Cap the number of tiled windows per desktop. Beyond the cap, new windows are made floating instead of being tiled. Can be set per desktop with *config -d*. Defaults to *false*.

'max_tiles_per_desktop'::
	The cap used when 'tile_limit_enabled' is set. A rule carrying *ignore_tile_limits=on* exempts matching windows. Defaults to *8*.
...
'allow_net_wm_moveresize'::
	Let clients start a move or a resize of their own window with *_NET_WM_MOVERESIZE*, as applications that draw their own title bar do. When *false*, *_NET_WM_MOVERESIZE* is not advertised, so those clients move and resize their windows themselves. Some clients, GTK among them, read the advertised list only once, so a change only reaches the windows they open afterwards. Defaults to *true*.
...
'ignore_monitor_updates'::
	Ignore monitor updates (RandR or wlroots output hotplug events) after the initial startup detection, freezing the monitor layout. Useful where a display or KVM switch reports spurious hotplugs. Note that this makes 'remove_disabled_monitors' and 'remove_unplugged_monitors' ineffective, since the events they act on are never processed. Can also be enabled from the start with the *-i* command line option. Defaults to *false*.
...
'BSPWM_SOCKET'::
	The path of the socket used for the communication between *bspc* and *bspwm*. If it isn't defined, then the following path is used: '$XDG_RUNTIME_DIR/bspwm<host_name>_<display_number>_<screen_number>-socket' (falling back to '/tmp/bspwm<host_name>_<display_number>_<screen_number>-socket' when '$XDG_RUNTIME_DIR' is unset).
